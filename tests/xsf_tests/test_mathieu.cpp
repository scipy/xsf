...
auto constexpr Degrees = xsf::mathieu::AngleUnitPolicy::Degrees;
...
TEST_CASE("sum_fourier_series_even_even_q1_m0_degrees", "[mathieu][xsf_tests]") {
    double q = 1.0;
    int m = 0;
    // Fourier coefficients
    std::vector<double> AA = {
    ...
    const std::vector<double> v = linspace(0.0, 180.0, 9);

    /* Reference values computed with Wolfram Engine. */
    std::vector<double> expected = {0.38482782930129905, 0.45675426482776166, 0.6543520522319161,
                                    0.8892092001491974,  0.9984585148130367,  0.8892092001491974,
                                    0.6543520522319161,  0.45675426482776166, 0.38482782930129905};
    std::vector<double> expected_diff = {0.0, 0.36076677721015715, 0.609537848441086,  0.5099312390005591,
                                         0.0, -0.5099312390005591, -0.609537848441086, -0.36076677721015715,
                                         0.0};

    for (decltype(v.size()) i = 0; i < v.size(); i++) {
        double out, out_diff;
        xsf::mathieu::sum_fourier_series<Even, Even, Degrees>(AA_span, v[i], out, out_diff);
        double rel_error = xsf::extended_relative_error(out, expected[i]);
        double rel_error_diff = xsf::extended_relative_error(out_diff, expected_diff[i]);
        CAPTURE(m, q, out, out_diff, expected[i], expected_diff[i], rel_error, rel_error_diff);
        REQUIRE(rel_error <= 1e-15);
...
TEST_CASE("sum_fourier_series_even_even_q1_m2_degrees", "[mathieu][xsf_tests]") {
    double q = 1.0;
    int m = 2;
    // Fourier coefficients
    std::vector<double> AA = {0.21692794675226462,     0.94825734682087837,     -0.081767008744172018,
    ...
    /* Reference values computed with Wolfram Engine. */
    std::vector<double> expected = {1.0859619368890114, 0.8856612185226889,  0.2986515729070593,
                                    -0.451718553340416, -0.8157268390500982, -0.451718553340416,
                                    0.2986515729070593, 0.8856612185226889,  1.0859619368890114};
    std::vector<double> expected_diff = {0.0, -1.0249411570023645, -1.8809997062068615, -1.6790771489646379,
                                         0.0, 1.6790771489646379,  1.8809997062068615,  1.0249411570023645,
                                         0.0};
...
TEST_CASE("sum_fourier_series_even_even_q1_m4_degrees", "[mathieu][xsf_tests]") {
    double q = 1.0;
    int m = 4;
    // Fourier coefficients
    std::vector<double> AA = {0.0052120044021059962,   0.083568404740583246,    0.99522416279527204,
    ...
    /* Reference values computed with wolfram engine. */
    std::vector<double> expected = {1.0351351585780133,   0.09855498844065812, -0.9889717294331822,
                                    -0.09021203125324906, 0.9678184211483233,  -0.09021203125324906,
                                    -0.9889717294331822,  0.09855498844065812, 1.0351351585780133};
    std::vector<double> expected_diff = {0.0, -3.8874689332135333, -0.4663987362342734, 4.074322044678967,
                                         0.0, -4.074322044678967,  0.4663987362342734,  3.8874689332135333,
                                         0.0};
...
TEST_CASE("sum_fourier_series_even_odd_q1_m1_degrees", "[mathieu][xsf_tests]") {
    double q = 1.0;
    int m = 1;
    // Fourier coefficients
    std::vector<double> AA = {
    ...
    std::vector<double> expected = {
        0.8565984655568863, 0.8592462553771193, 0.794471810010097,   0.5134490713977,    0.0,
        -0.5134490713977,   -0.794471810010097, -0.8592462553771193, -0.8565984655568863
    };

    std::vector<double> expected_diff = {
        0.0,
        -0.019785018583110214,
        -0.3835394770372131,
        -1.0626058713019821,
        -1.439819078636166,
        -1.0626058713019821,
    ...
    for (decltype(v.size()) i = 0; i < v.size(); i++) {
        ...
        double rtol_diff = (i != 8) ? 1e-14 : 5e-4;
        double out, out_diff;
        xsf::mathieu::sum_fourier_series<Even, Odd, Degrees>(AA_span, v[i], out, out_diff);
        double rel_error = xsf::extended_relative_error(out, expected[i]);
        double rel_error_diff = xsf::extended_relative_error(out_diff, expected_diff[i]);
        CAPTURE(m, q, i, out, out_diff, expected[i], expected_diff[i], rel_error, rel_error_diff);
        REQUIRE(rel_error <= rtol);
...
TEST_CASE("sum_fourier_series_even_odd_q1_m3_degrees", "[mathieu][xsf_tests]") {
    double q = 1.0;
    int m = 3;
    // Fourier coefficients
    std::vector<double> AA = {0.1396156546048497,     0.98825110013683626,     -0.06216755513992854,
    ...
    /* Reference values computed with wolfram engine. */
    std::vector<double> expected = {
        1.067235512293139,  0.5295465974599358, -0.5550307133804575, -0.9176193661208333, 0.0,
        0.9176193661208333, 0.5550307133804575, -0.5295465974599358, -1.067235512293139
    };
...
TEST_CASE("sum_fourier_series_odd_even_q1_m2_degrees", "[mathieu][xsf_tests]") {
    double q = 1.0;
    int m = 2;
    // Fourier coefficients
    std::vector<double> BB = {
    ...
    /* Reference values computed with wolfram engine. */
    std::vector<double> expected = {0.0, 0.6238151115773868,  0.9939936206449127,  0.7891966670415149,
                                    0.0, -0.7891966670415149, -0.9939936206449127, -0.6238151115773868,
                                    0.0};
    std::vector<double> expected_diff = {1.6775141712238477,  1.3987651018998324, 0.3304197450717404,
                                         -1.3980782681209842, -2.339727328933457, -1.3980782681209842,
                                         0.3304197450717404,  1.3987651018998324, 1.6775141712238477};

    for (decltype(v.size()) i = 0; i < v.size(); i++) {
        double out, out_diff;
        xsf::mathieu::sum_fourier_series<Odd, Even, Degrees>(BB_span, v[i], out, out_diff);
        double rel_error = xsf::extended_relative_error(out, expected[i]);
        double rel_error_diff = xsf::extended_relative_error(out_diff, expected_diff[i]);
        CAPTURE(m, q, out, out_diff, expected[i], expected_diff[i], rel_error, rel_error_diff);
        REQUIRE(rel_error <= 1e-15);
...
TEST_CASE("sum_fourier_series_odd_even_q1_m4_degrees", "[mathieu][xsf_tests]") {
    double q = 1.0;
    int m = 4;
    // Fourier coefficients
    std::vector<double> BB = {
    ...
    /* Reference values computed with wolfram engine. */
    std::vector<double> expected = {0.0, 1.018535489113575,  0.13260429952450176,  -0.9721093212091416,
                                    0.0, 0.9721093212091416, -0.13260429952450176, -1.018535489113575,
                                    0.0};
    std::vector<double> expected_diff = {3.8555218154420925,  0.3204511230639427, -3.972966651023333,
                                         -0.3371001615212428, 4.123709563806253,  -0.3371001615212428,
                                         -3.972966651023333,  0.3204511230639427, 3.8555218154420925};
...
TEST_CASE("sum_fourier_series_odd_odd_q1_m1_degrees", "[mathieu][xsf_tests]") {
    double q = 1.0;
    int m = 1;
    // Fourier coefficients
    std::vector<double> BB = {0.99396796139893406,     -0.109583791872277,      0.0043676488669854262,
    ...
    std::vector<double> expected_diff = {
        0.686441897503372,   0.784708039274585,   0.9194285732565753, 0.7045120739186153, 0.0,
        -0.7045120739186153, -0.9194285732565753, -0.784708039274585, -0.686441897503372
    };
    ...
    for (decltype(v.size()) i = 0; i < v.size(); i++) {
        ...
        double rtol_diff = (i != 4) ? 1e-14 : 1e-5;
        double out, out_diff;
        xsf::mathieu::sum_fourier_series<Odd, Odd, Degrees>(BB_span, v[i], out, out_diff);
        double rel_error = xsf::extended_relative_error(out, expected[i]);
        double rel_error_diff = xsf::extended_relative_error(out_diff, expected_diff[i]);
        CAPTURE(m, q, i, out, out_diff, expected[i], expected_diff[i], rel_error, rel_error_diff);
        REQUIRE(rel_error <= rtol);
...
TEST_CASE("sum_fourier_series_odd_odd_q1_m3_degrees", "[mathieu][xsf_tests]") {
    double q = 1.0;
    int m = 3;
    // Fourier coefficients
    std::vector<double> BB = {0.10964247330123811,     0.9920165102302132,     -0.062284339384168448,
    ...
    std::vector<double> expected_diff = {
        2.784993910508225, 1.3494510247766995, -1.7990677315460224, -2.999369976687616, 0.0,
        2.999369976687616, 1.7990677315460224, -1.3494510247766995, -2.784993910508225
    };
