...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "cyl_hankel_1e"};

TEST_CASE("cyl_hankel_1e dD->D scipy_special_tests", "[cyl_hankel_1e][dD->D][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    auto out = xsf::cyl_hankel_1e(v, z);
