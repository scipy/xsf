...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "poch"};

TEST_CASE("cephes::poch dd->d scipy_special_tests", "[cephes::poch][dd->d][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    auto out = xsf::cephes::poch(z, m);
    auto error = xsf::extended_relative_error(out, desired);
    tol = adjust_tolerance(tol);
    CAPTURE(z, m, out, desired, error, tol, fallback);
    REQUIRE(error <= tol);
}
