...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "prolate_aswfa_nocv"};

TEST_CASE("prolate_aswfa_nocv dddd->dd scipy_special_tests", "[prolate_aswfa_nocv][dddd->dd][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    xsf::prolate_aswfa_nocv(m, n, c, x, out0, out1);
    auto [tol0, tol1] = tol;
