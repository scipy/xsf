...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "oblate_radial2_nocv"};

TEST_CASE("oblate_radial2_nocv dddd->dd scipy_special_tests", "[oblate_radial2_nocv][dddd->dd][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    xsf::oblate_radial2_nocv(m, n, c, x, out0, out1);
    auto [tol0, tol1] = tol;
