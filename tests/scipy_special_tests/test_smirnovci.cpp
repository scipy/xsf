...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "smirnovci"};

TEST_CASE("smirnovci pd->d scipy_special_tests", "[smirnovci][pd->d][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    auto out = xsf::cpu::smirnovci(n, p);
