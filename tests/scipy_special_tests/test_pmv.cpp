...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "pmv"};

TEST_CASE("pmv ddd->d scipy_special_tests", "[pmv][ddd->d][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    auto out = xsf::pmv(m, v, x);
    auto error = xsf::extended_relative_error(out, desired);
    tol = adjust_tolerance(tol);
    CAPTURE(m, v, x, out, desired, error, tol, fallback);
    REQUIRE(error <= tol);
}
