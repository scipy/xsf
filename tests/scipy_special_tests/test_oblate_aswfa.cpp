...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "oblate_aswfa"};

TEST_CASE("oblate_aswfa ddddd->dd scipy_special_tests", "[oblate_aswfa][ddddd->dd][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    xsf::oblate_aswfa(m, n, c, cv, x, out0, out1);
    auto [tol0, tol1] = tol;
