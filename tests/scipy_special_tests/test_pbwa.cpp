...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "pbwa"};

TEST_CASE("pbwa dd->dd scipy_special_tests", "[pbwa][dd->dd][scipy_special_tests]") {
    SET_FP_FORMAT()
