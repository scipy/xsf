...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "wright_bessel"};

TEST_CASE("wright_bessel ddd->d scipy_special_tests", "[wright_bessel][ddd->d][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    auto out = xsf::wright_bessel(a, b, x);
