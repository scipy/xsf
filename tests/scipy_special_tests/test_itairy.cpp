...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "itairy"};

TEST_CASE("itairy d->dddd scipy_special_tests", "[itairy][d->dddd][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    xsf::itairy(x, out0, out1, out2, out3);
