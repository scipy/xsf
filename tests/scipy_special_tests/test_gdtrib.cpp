...
fs::path tables_path{fs::path(XSREF_TABLES_PATH) / "scipy_special_tests" / "gdtrib"};

TEST_CASE("gdtrib ddd->d scipy_special_tests", "[gdtrib][ddd->d][scipy_special_tests]") {
    SET_FP_FORMAT()
    ...
    auto out = xsf::gdtrib(a, p, x);
    auto error = xsf::extended_relative_error(out, desired);
    tol = adjust_tolerance(tol);
    CAPTURE(a, p, x, out, desired, error, tol, fallback);
    REQUIRE(error <= tol);
}
