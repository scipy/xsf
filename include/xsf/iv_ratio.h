...
struct IvRatioCFTailGenerator {
    ...
    XSF_HOST_DEVICE cxx::pair<T, T> operator()() noexcept {
        using cxx::fma;
        ++k_;
...
XSF_HOST_DEVICE inline cxx::pair<double, cxx::uint64_t> _iv_ratio_cf(double v, double x, bool complement) {
    ...
    auto [fc, terms] = detail::series_eval_kahan(
        detail::continued_fraction_series(cf), T(cxx::numeric_limits<double>::epsilon()), 1000, 2 * vc
    );
...
XSF_HOST_DEVICE inline double iv_ratioinv(double v, double r) {
    if (cxx::isnan(v) || cxx::isnan(r)) {
        return cxx::numeric_limits<double>::quiet_NaN();
    }
    if (!cxx::isfinite(v) || v < 0.5 || r < 0.0 || r > 1.0) {
    ...
    double vm1 = v - 1.0;
    double lower_bound = r * (vm1 + cxx::sqrt(vm1 * vm1 + 4.0 * v * one_minus_r_sq)) / one_minus_r_sq;

    double vm05 = v - 0.5;
    double upper_bound = r * (vm05 + cxx::sqrt(vm05 * vm05 + 2.0 * v * one_minus_r_sq)) / one_minus_r_sq;
    ...
    if (f_xl * f_xr > 0) {
        ...
        auto [b_xl, b_xr, b_f_xl, b_f_xr, bracket_status] = detail::bracket_root_for_cdf_inversion(
            func, 1.0, cxx::numeric_limits<double>::min(), cxx::numeric_limits<double>::max(), -0.875, 7.0, 0.125, 8,
            true, 342
        );
        if (bracket_status == 1) {
    ...
    auto [result, root_status] =
        detail::find_root_chandrupatla(func, xl, xr, f_xl, f_xr, cxx::numeric_limits<double>::epsilon(), 1e-308, 1000);
    if (root_status) {
