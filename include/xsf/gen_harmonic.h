...
namespace detail {
    ...
    // than 5e-15.
    //
    inline constexpr double zeta_ratio_threshold = 0.9;
...
inline double normalized_gen_harmonic(T j, T k, T n, double a) {
    ...
    if (j < 1 || k < j || n < k) {
        set_error(
            "_normalized_gen_harmonic", SF_ERROR_DOMAIN,
    ...
    if (a <= 1) {
        return detail::normalized_sum_powers(j, k, n, a);
    } else {
        ...
        double z1 = xsf::cephes::zeta(a, static_cast<double>(1));
        double znp1 = xsf::cephes::zeta(a, static_cast<double>(n + 1));
        bool zeta_numer_ok = zkp1 / zj < detail::zeta_ratio_threshold;
        bool zeta_denom_ok = znp1 / z1 < detail::zeta_ratio_threshold;
        if (zeta_numer_ok) {
            if (zeta_denom_ok) {
            ...
            if (zeta_denom_ok) {
                return detail::sum_powers(j, k, a) / (z1 - znp1);
            } else {
                return detail::normalized_sum_powers(j, k, n, a);
            }
        }
