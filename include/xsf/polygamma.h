...
namespace detail {
    ...
    polygamma_asymptotic_series(int n, std::complex<double> z) {
        ...
        */
        static constexpr std::array<double, 48> bernoulli2k = {
            ...
            5.6920695482035283174e+48, -8.2183629419784577665e+50, 1.2502904327166994004e+53, 
            -2.0015583233248370052e+55, 3.3674982915364375556e+57, -5.9470970503135450205e+59, 
            1.1011910323627976762e+62, -2.1355259545253502079e+64, 4.3328896986641193847e+66, 
            -9.1885528241669331811e+68, 2.0346896776329073708e+71, -4.7003833958035730158e+73,
        };

        std::complex<double> res, term, fac_coeff;
        std::complex<double> const zn_inv = std::pow(z, static_cast<std::complex<double>>(-n));
        ...
        for (int k=1;k<bernoulli2k.size()+1;k++) {
            zfac *= rzz;
            fac_coeff *= static_cast<std::complex<double>>((2*k+n-2)*(2*k+n-1))/static_cast<std::complex<double>>(2*k*(2*k-1));
    ...
    XSF_HOST_DEVICE inline std::complex<double> 
    polygamma_reflection_rhs_nleq15(int n, std::complex<double> z) {
         ...
         * However, the coefficients blow up very rapidly in n, reaching ~1E+13 for n=15.
         * For larger n, there is little point using this strategy. 
         * Recurrence should be used instead.
         * Alternatively, we are going to divide this result by n!. So one could divide by (k+1)
         * within the loop. However, one would need to consider a dynamical allocation strategy.
         */
        
        std::complex<double> cot_piz = 1. / std::tan(M_PI * z);
        if (n == 0) {
            return M_PI * cot_piz;
        }

        if(n>15){
            // We are not going to use it for n>15.
            // Statically allocate the memory and check against misuse
            return std::numeric_limits<double>::quiet_NaN();
        }

        std::array<std::complex<double>, 17> coefficient_array{}; // all zeros at the start
        std::array<std::complex<double>, 17> next_coeffs; 
        ...
        for(int k=0; k<n; k++){
            ...
            for(int m=0; m<k+3; m++){
                std::complex<double> term1 = (m + 1 <= k + 1) ? 
                    (-static_cast<std::complex<double>>(m + 1) * coefficient_array[m + 1]) :
                    0.;
                std::complex<double> term2 = (m - 1 >= 0) ? 
                    (-static_cast<std::complex<double>>(m - 1) * coefficient_array[m - 1]) :
                    0.;
                next_coeffs[m] = term1 + term2;
            }
...
XSF_HOST_DEVICE inline std::complex<double> polygamma(int n, std::complex<double> z) {
    ...
    if (n <= 15){ 
        ...
        if(z.real() < 0 && std::abs(z.imag()) < smallabsz) {
            std::complex<double> const reflection_rhs = detail::polygamma_reflection_rhs_nleq15(n, z);
            res -= reflection_rhs / gamma(static_cast<double>(n + 1));
            sign *= (n & 1) ? -1. : 1.;
            z = 1. - z;
            absz = std::abs(z);
        } 
